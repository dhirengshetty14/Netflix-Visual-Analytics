...
const ORDER: Movie["rating_group"][] = ["Kids", "Teen", "Adult", "Other"];

export default function MoviesRuntime({
  data,
  onBrush, // <- callback to app: range [min,max] or null
}: {
  data: Movie[];
  ...
  // compute box stats per rating group
  const stats = useMemo(() => {
    const by = d3.group(
      data.filter(d => d.runtime_min != null),
      d => d.rating_group
    );
    return groups.map(g => {
      const arr = (by.get(g) ?? []).map(d => d.runtime_min as number).sort(d3.ascending);
  ...
  useEffect(() => {
    ...
    const y = d3.scaleLinear()
      .domain([0, d3.max(stats, s => s.high) ?? 200])
      .nice()
      .range([h, 0]);

    const color = d3.scaleOrdinal<string, string>()
      .domain(groups)
      .range(d3.schemeSet2 as any);
    ...
    // jitter sample (retain titles for tooltips)
    ...
    for (const g of groups) {
      const arr = (byGroup.get(g) ?? []).map(d => ({ x: g, y: d.runtime_min as number, title: d.title }));
      const step = Math.max(1, Math.ceil(arr.length / 250)); // cap ~250 points
    ...
    const points = root.append("g")
      ...
      .on("mouseenter", function (event, d) {
        ...
        tip.style("opacity", "1")
           .html(`<b>${d.title}</b><div>${d.y} min</div>`);
      })
    ...
    // highlight on hover (dims others & shows numbers)
    ...
    function showLabel(s: (typeof stats)[number]) {
      ...
      label.append("text")
        ...
        .attr("fill", "#fff")
        .attr("font-size", 11)
        .text(`${s.group}: Q2 ${Math.round(s.q2)}m`);
    }
    ...
    root.append("text")
      ...
      .attr("fill", "white")
      .attr("font-size", 12)
      .text("Movie runtimes (minutes) by rating");

    // --- BRUSH (Y) ---
    const brush = d3.brushY()
      .extent([[w + 6, 0], [w + 16, h]]) // slim brush rail at right
      .on("end", (event) => {
